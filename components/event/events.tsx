'use client'
import { EventsModal } from "@/app/utils/modals";
  ...
  return (
    <>
      ...
      setOpen={setOpen} >
        ...
        imageMode="conatin"
        imageData = {events?.rows?.[imageIndex]?.images?.map((ele, index)=> ({id:index+1,title: events?.rows?.[0]?.title, imageUrl: ele?.imageUrl}))}/>
        </div>
      </CustomDialog>
      </>
  )
